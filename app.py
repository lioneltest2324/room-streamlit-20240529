...
ads_daily_filtered_date_range_1_7_df = create_date_filtered_df(ads_daily, 'Date', fix_selected_range)
merge_1_7_df = output_groupby_df(ads_daily_filtered_date_range_1_7_df, ['SKU'],
['impression', 'click', 'cost', 'ads value', 'conversions', ], 'sum').reset_index()
merge_1_7_df = add_custom_proportion_to_df_x100(merge_1_7_df,'click','impression','CTR')
merge_1_7_df = add_custom_proportion_to_df(merge_1_7_df,'ads value','cost','ads ROI')

merge_1_7_df = merge_1_7_df.rename(columns={
...
enddate_date = datetime.strptime(enddate, '%Y-%m-%d').date()
fix_selected_range = (startdate_date, enddate_date)
ads_daily_filtered_date_range_8_15_df = create_date_filtered_df(ads_daily, 'Date', fix_selected_range)
merge_8_15_df = output_groupby_df(ads_daily_filtered_date_range_8_15_df, ['SKU'],
['impression', 'click', 'cost', 'ads value', 'conversions', ], 'sum').reset_index()
merge_8_15_df = add_custom_proportion_to_df_x100(merge_8_15_df,'click','impression','CTR')
merge_8_15_df = add_custom_proportion_to_df(merge_8_15_df,'ads value','cost','ads ROI')

merge_8_15_df = merge_8_15_df.rename(columns={
'impression': f'impression({start_formatted_date}-{end_formatted_date})',
'click': f'click({start_formatted_date}-{end_formatted_date})',
'cost': f'cost({start_formatted_date}-{end_formatted_date})',
'ads value': f'ads value({start_formatted_date}-{end_formatted_date})',
...
enddate_date = datetime.strptime(enddate, '%Y-%m-%d').date()
fix_selected_range = (startdate_date, enddate_date)
ads_daily_filtered_date_range_16_22_df = create_date_filtered_df(ads_daily, 'Date', fix_selected_range)
merge_16_22_df = output_groupby_df(ads_daily_filtered_date_range_16_22_df, ['SKU'],
['impression', 'click', 'cost', 'ads value', 'conversions', ], 'sum').reset_index()
merge_16_22_df = add_custom_proportion_to_df_x100(merge_16_22_df,'click','impression','CTR')
merge_16_22_df = add_custom_proportion_to_df(merge_16_22_df,'ads value','cost','ads ROI')

merge_16_22_df = merge_16_22_df.rename(columns={
'impression': f'impression({start_formatted_date}-{end_formatted_date})',
'click': f'click({start_formatted_date}-{end_formatted_date})',
'cost': f'cost({start_formatted_date}-{end_formatted_date})',
'ads value': f'ads value({start_formatted_date}-{end_formatted_date})',
...
startdate = twentynine_date
enddate = twentythree_date
start_formatted_date = datetime.strptime(startdate, '%Y-%m-%d').strftime('%m-%d')
end_formatted_date = datetime.strptime(enddate, '%Y-%m-%d').strftime('%m-%d')
...
enddate_date = datetime.strptime(enddate, '%Y-%m-%d').date()
fix_selected_range = (startdate_date, enddate_date)
ads_daily_filtered_date_range_23_29_df = create_date_filtered_df(ads_daily, 'Date', fix_selected_range)
merge_23_29_df = output_groupby_df(ads_daily_filtered_date_range_23_29_df, ['SKU'],
['impression', 'click', 'cost', 'ads value', 'conversions', ], 'sum').reset_index()
merge_23_29_df = add_custom_proportion_to_df_x100(merge_23_29_df,'click','impression','CTR')
merge_23_29_df = add_custom_proportion_to_df(merge_23_29_df,'ads value','cost','ads ROI')

merge_23_29_df = merge_23_29_df.rename(columns={
'impression': f'impression({start_formatted_date}-{end_formatted_date})',
'click': f'click({start_formatted_date}-{end_formatted_date})',
'cost': f'cost({start_formatted_date}-{end_formatted_date})',
'ads value': f'ads value({start_formatted_date}-{end_formatted_date})',
...
merged_df_left = pd.merge(merge_23_29_df, merge_16_22_df, on='SKU', how='left')
merged_df_left = pd.merge(merged_df_left, merge_8_15_df, on='SKU', how='left')
merged_df_left = pd.merge(merged_df_left, merge_1_7_df, on='SKU', how='left')
...
merged_df_left = merged_df_left.filter(regex='SKU|SPU|scenes|imagelink|product_type_1|product_type_2|product_type_3|impression|click|cost|ads value|conversions|CTR|ads ROI')
